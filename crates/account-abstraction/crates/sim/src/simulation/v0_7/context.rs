...
use aa_contracts::v0_7::ValidationResult;
use aa_provider::{BlockId, EntryPoint, EvmProvider, SimulationProvider};
use aa_types::{
        ...
        match top.exit_type {
            ExitType::Revert => {
                let bytes: Bytes = hex::decode(top.exit_data.clone())
                    ...
                    .into();
                Ok(Err(aa_provider::decode_v0_7_validation_revert(&bytes)))
            }
            ExitType::Return => {
