...
use aa_provider::{EvmProvider, FeeEstimator};
use aa_types::{
...
{
    #[instrument(skip_all, fields(rpc_method = "aa_maxPriorityFeePerGas"))]
    async fn max_priority_fee_per_gas(&self) -> RpcResult<U128> {
        utils::safe_call_rpc_handler(
            "aa_maxPriorityFeePerGas",
            RundlerApi::max_priority_fee_per_gas(self),
        )
        .await
    }

    #[instrument(skip_all, fields(rpc_method = "aa_dropLocalUserOperation"))]
    async fn drop_local_user_operation(
        &self,
        ...
        utils::safe_call_rpc_handler(
            "aa_dropLocalUserOperation",
            RundlerApi::drop_local_user_operation(self, user_op, entry_point),
        )
        .await
    }

    #[instrument(skip_all, fields(rpc_method = "aa_getMinedUserOperation"))]
    async fn get_mined_user_operation(
        &self,
        uo_hash: B256,
        tx_hash: B256,
        ...
        utils::safe_call_rpc_handler(
            "aa_getMinedUserOperation",
            RundlerApi::get_mined_user_operation(self, uo_hash, tx_hash, entry_point),
        )
        .await
    }

    #[instrument(skip_all, fields(rpc_method = "aa_getUserOperationStatus"))]
    async fn get_user_operation_status(&self, uo_hash: B256) -> RpcResult<RpcUserOperationStatus> {
        utils::safe_call_rpc_handler(
            "aa_getUserOperationStatus",
    ...
    #[instrument(
        skip_all,
        fields(rpc_method = "aa_getPendingUserOperationBySenderNonce")
    )]
    async fn get_pending_user_operation_by_sender_nonce(
        &self,
        sender: Address,
        ...
        utils::safe_call_rpc_handler(
            "aa_getPendingUserOperationBySenderNonce",
