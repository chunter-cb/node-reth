...
use aa_contracts::multicall3::{self, Multicall3::Multicall3Instance};
use aa_types::{
    chain::ChainSpec,
    da::{BedrockDAGasBlockData, BedrockDAGasData, DAGasBlockData, DAGasData},
};
use aa_utils::cache::LruMap;
    ...
    AP: AlloyProvider,
{
    ...
    async fn get_gas_data(&self, data: Bytes) -> ProviderResult<BedrockDAGasData> {
        ...
        let compressed_len = self
            .blocking_task_pool
            .spawn(move || {
                let _buf = vec![0; data.len() * 2];
                // TODO: Replace with actual fastlz compression
                // For now, approximate compressed size as 50% of original
                data.len() as u64 / 2
            })
            .await
